...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    # Freeze p1loss head (opponent policy) for adversarial training
    # Using gradient hooks to freeze only channel 1
    def freeze_p1loss_channel(model):
        # Freeze conv2p channel 1 (opponent policy) using gradient hook
        mask_conv2p = torch.ones_like(model.policy_head.conv2p.weight)
        if model.policy_head.conv2p.weight.shape[0] > 1:
            mask_conv2p[1] = 0.0  # Freeze channel 1
        model.policy_head.conv2p.weight.register_hook(lambda grad: grad * mask_conv2p)

        # Freeze linear_pass channel 1 using gradient hook
        mask_linear_pass = torch.ones_like(model.policy_head.linear_pass.weight)
        if model.policy_head.linear_pass.weight.shape[0] > 1:
            mask_linear_pass[1] = 0.0  # Freeze channel 1
        model.policy_head.linear_pass.weight.register_hook(lambda grad: grad * mask_linear_pass)

        # Freeze linear_pass2 channel 1 if it exists
        if hasattr(model.policy_head, 'linear_pass2'):
            mask_linear_pass2 = torch.ones_like(model.policy_head.linear_pass2.weight)
            if model.policy_head.linear_pass2.weight.shape[0] > 1:
                mask_linear_pass2[1] = 0.0  # Freeze channel 1
            model.policy_head.linear_pass2.weight.register_hook(lambda grad: grad * mask_linear_pass2)

        # Freeze intermediate_policy_head if it exists
        if hasattr(model, 'intermediate_policy_head'):
            mask_conv2p_inter = torch.ones_like(model.intermediate_policy_head.conv2p.weight)
            if model.intermediate_policy_head.conv2p.weight.shape[0] > 1:
                mask_conv2p_inter[1] = 0.0
            model.intermediate_policy_head.conv2p.weight.register_hook(lambda grad: grad * mask_conv2p_inter)

            mask_linear_pass_inter = torch.ones_like(model.intermediate_policy_head.linear_pass.weight)
            if model.intermediate_policy_head.linear_pass.weight.shape[0] > 1:
                mask_linear_pass_inter[1] = 0.0
            model.intermediate_policy_head.linear_pass.weight.register_hook(lambda grad: grad * mask_linear_pass_inter)

            if hasattr(model.intermediate_policy_head, 'linear_pass2'):
                mask_linear_pass2_inter = torch.ones_like(model.intermediate_policy_head.linear_pass2.weight)
                if model.intermediate_policy_head.linear_pass2.weight.shape[0] > 1:
                    mask_linear_pass2_inter[1] = 0.0
                model.intermediate_policy_head.linear_pass2.weight.register_hook(lambda grad: grad * mask_linear_pass2_inter)

        logging.info("Frozen p1loss head (opponent policy channel 1) using gradient hooks for adversarial training")

    # Freeze p1loss BEFORE creating optimizer so frozen params are not added to param groups
    freeze_p1loss_channel(raw_model)
