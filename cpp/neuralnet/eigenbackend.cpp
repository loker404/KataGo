...
#define MAP2(NAME) TENSORMAP2 NAME(buffers.NAME.data(), buffers.NAME.dimension(0), batchSize)
  ...
  for(int row = 0; row < batchSize; row++) {
    ...
    if(numPolicyChannels == 4 && modelVersion >= 16) {
    ...
    else if(numPolicyChannels == 2) {
      // For 2-channel models (version >= 12 && version < 16):
      // Channel 0: policy
      // Channel 1: opponent policy (p1loss) - for adversarial training
      for(int i = 0; i<nnXLen*nnYLen; i++) {
        float p = policySrcBuf[i*numPolicyChannels];
        float pOpponent = policySrcBuf[i*numPolicyChannels+1];  // Channel 1 is opponent policy
      ...
      opponentPolicyProbs[inputBuffers->singlePolicyResultElts] = policyPassSrcBuf[0];  // Opponent pass policy
    }
